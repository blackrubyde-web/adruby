...
const AdRubyPaymentVerification = () => {
  ...
  const { user, loading, subscriptionStatus, isSubscribed, refreshSubscriptionStatus, isAuthReady } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!isAuthReady) {
      console.log('[PaymentVerification] waiting for auth…');
      return;
    }

    if (!user) {
      console.log('[PaymentVerification] no session → redirect /ad-ruby-registration');
      navigate('/ad-ruby-registration', { replace: true });
      return;
    }

    if (isSubscribed?.()) {
      console.log('[PaymentVerification] already subscribed → redirect /dashboard');
      navigate('/overview-dashboard', { replace: true });
      return;
    }

    console.log('[PaymentVerification] show payment UI (no active subscription)');
  }, [user, isAuthReady, onboardingStatus, isOnboardingComplete, navigate]);
