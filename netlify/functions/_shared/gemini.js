...
function buildAdPrompt({ headline, subheadline, cta, productAnalysis, style, referencePattern, enhancedPrompt }) {
    ...
    return `PROFESSIONAL PRODUCT ADVERTISEMENT PHOTO

Create a photorealistic advertisement image (1080x1080px) that looks like it was shot by a professional photographer, NOT generated by AI.
...
═══════════════════════════════════════
PHOTOREALISTIC QUALITY (CRITICAL)
═══════════════════════════════════════
This must look like a REAL PHOTOGRAPH, not AI-generated:
• Real studio photography lighting with natural falloff
• Authentic material textures (not plastic/fake looking)
• Subtle imperfections that real photos have
• Professional color grading like high-end advertising

═══════════════════════════════════════
SCENE SETUP
═══════════════════════════════════════
• Background: Dark gradient, almost black at edges
• Surface: Subtle reflective surface under product
• Lighting: Key light from upper left, fill from right
• Atmosphere: Slight haze/atmosphere for depth
• Product: Centered, filling 50% of frame

═══════════════════════════════════════
TEXT ELEMENTS (EXACTLY 3 - NO MORE!)
═══════════════════════════════════════
1. HEADLINE (top center): "${headlineText}"
   - Bold white text, large
   - One line only

2. ${subheadline ? `TAGLINE (below headline): "${subheadline}"
   - Smaller, light gray
   - One line only` : 'No tagline'}

3. CTA BUTTON (bottom center): "${ctaText}"
...
STRICT RULES
═══════════════════════════════════════
✗ NO duplicate text lines
✗ NO corrupted or gibberish text
✗ NO extra text beyond the 3 elements above
✗ NO plastic/fake looking materials
✗ NO obviously AI-generated artifacts
✗ NO feature bullets or icons (keep it minimal)

OUTPUT: A premium, photorealistic product ad that could be in a magazine.`;
