...
export const handler = async (event) => {
    // ── OUTER SAFETY NET — catch anything that crashes the worker ──
    try {
        ...
        if (!process.env.GEMINI_API_KEY) {
            console.error('[AI Ad Generate] FATAL: GEMINI_API_KEY environment variable is not set');
            return { statusCode: 500, headers, body: JSON.stringify({ error: 'Server configuration error', message: 'KI-Service nicht konfiguriert. Bitte kontaktiere den Support.' }) };
        }
        ...
        // ── OUTER SAFETY NET — prevents Netlify worker crash ──
        console.error('[AI Ad Generate] OUTER CRASH:', outerError);
        ...
        return {
            ...
            body: JSON.stringify({
                error: 'Ad generation failed',
                message: outerError?.message || 'Interner Serverfehler. Bitte versuche es erneut.',
                recoverable: true,
            }),
        };
    }
